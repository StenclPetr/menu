<?php
// source: C:\xampp\htdocs\menu\app\presenters/templates/AddJidlopage/add.latte

use Latte\Runtime as LR;

class Template8ecf1e789d extends Latte\Runtime\Template
{
	public $blocks = [
...
<?php
	...
	{
		extract($this->params);
		if (isset($this->params['gen_alergen'])) trigger_error('Variable $gen_alergen overwritten in foreach on line 30');
...
<?php
		...
		echo $_input->getLabelPart()->attributes() ?>>Název</label>
                                    <input class="form-control" placeholder="Název jídla"<?php
		$_input = end($this->global->formsStack)["nazev"];
		...
		echo $_input->getLabelPart()->attributes() ?>>Datum</label>
                                    <input id="datum" class="form-control" placeholder="Datum"<?php
		$_input = end($this->global->formsStack)["datum"];
		echo $_input->getControlPart()->addAttributes(array (
		'id' => NULL,
		'class' => NULL,
...
?>
			...
			'class' => NULL,
			'for' => NULL,
			))->attributes() ?>><?php echo LR\Filters::escapeHtmlText($gen_alergen->nazev) /* line 33 */ ?></label>
                                            </div>
<?php
			$iterations++;
		}
